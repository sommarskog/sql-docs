...
- **Fallback DNS resolution**: Managed instances depend on valid DNS resolution in their virtual networks. If a managed instance's virtual network is configured to use [custom DNS server(s)](/azure/virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances#name-resolution-that-uses-your-own-dns-server) and a DNS request fails to complete within one second, managed instance will repeat the request against [Azure DNS's resolvers](/azure/virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances#azure-provided-name-resolution). This circumvents nonfunctional or nonresponsive custom DNS servers.
