...
The desired state of the configuration setting. *@option_value* is **varchar(60)** with no defaults. Possible values are `ON` or `OFF`. For the `FORCE_LAST_GOOD_PLAN` option, setting the value to `ON` will cause APC to ignore a `query_id` in the Query Store from being monitored by the system for query plan regressions.
